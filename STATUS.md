...
| **B'** | グラフ検出器（`export_hits_grid`→`detectlseg`→`integrate_smallregions`→`detectbunki`） | 不使用 | **移植完了（3段とも）**。真値で eff 96.2% / purity 74.0%。**export の格子を 30→6 px に変更**（実データコストは未測定）|
...
シミュレーションは 159本/view、実 E07 はもっと密。**エクスポートを
本番で回す前に 1 tile で実測すること。**
...
- **実 E07 タイル 1 枚を `--cell-px 6` でエクスポートしてコストを実測**
  （現在の最優先）。6 px は 30 px より約25倍の点数になる。ここが通れば
  グラフ検出器を実データに向けられる。
